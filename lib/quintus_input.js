...
Quintus.Input = function(Q) {
    ...
    drawButtons: function() {
      ...
      for(var i=0;i<keypad.controls.length;i++) {
        ...
        if(control[0]) {
          ...
          ctx.font = "bold " + (keypad.size/1.5) + "px Zeldicons";
  ...
  Q.controls = function(joypad) {
    ...
    if(joypad) {
      Q.input.touchControls({
        ...
        //controls: [ [],[],[],['previousItem','<' ],['nextItem','>' ],['action','B'],['fire','A']]
        controls: [ [],[],[],['previousItem','«' ],['action','\ue600'],['nextItem','»' ],['fire','\ue612']]
      });
  ...
  Q.component("stepControls", {
    ...
    /**
   * Top-Down Controls Component
   *
   * Adds 2D platformer 4d controls onto a Sprite
   *
   * Platformer controls bind to left, and right and allow the player to jump.
   *
   * Adds the following properties to the entity to control speed and jumping:
  ...
  Q.component("topdownControls", {
    ...
    added: function() {
      ...
      p.midSlash = false;
      p.justSlashEnd = true;
      p.holdingItemChange = false;

      p.justPressedAction = false;
      p.holdingAction = false;
    },
    ...
    step: function(dt) {
      ...
      if(!p.holdingItemChange && Q.inputs['nextItem']) 
        Q.state.inc("currentItem",1);
      
      else if (!p.holdingItemChange && Q.inputs['previousItem'])
        Q.state.dec("currentItem",1);
      

      p.holdingItemChange = Q.inputs['nextItem'] || Q.inputs['previousItem'];

      if(p.ignoreControls === undefined || !p.ignoreControls) {
        var collision = null;

        // Follow along the current slope, if possible.
        if(p.collisions !== undefined && p.collisions.length > 0 && (Q.inputs['left'] || Q.inputs['right'] || Q.inputs['up'] || Q.inputs['down'])) {
          if(p.collisions.length === 1) {
            collision = p.collisions[0];
          } else {
            // If there's more than one possible slope, follow slope with negative Y normal
        ...
        var anyMove = false;
        var multipleMoves = false;
        p.lastdirection = p.direction;

        if(Q.inputs['action']){
          if(p.justPressedAction){
            p.justPressedAction = false;
            p.holdingAction = true;
          }
          else if (!p.holdingAction)
            p.justPressedAction = true;
        }
        else{
          p.justPressedAction = false;
          p.holdingAction = false;
        ...
        if(p.holdingSword && !Q.inputs['fire'])
          p.holdingSword = false;

        if(p.justSlashEnd){
          p.justSlashEnd = false;
          p.holdingSword = Q.inputs['fire'];
        } 
        else if (!p.holdingSword){
          if(!p.midSlash && !p.newSlash && Q.inputs['fire']) {
            p.newSlash = true;
          } 
        }
        ...
        else if (multipleMoves && p.lastdirection != '')
          p.direction = p.lastdirection;

        if(p.ignoreControlsSoft){ //restore knockback speed
          p.vx = oldSpeedX;
          p.vy = oldSpeedY;
        }
