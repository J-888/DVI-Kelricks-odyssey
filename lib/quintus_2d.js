...
Quintus["2D"] = function(Q) {
 ...
 Q.Sprite.extend("TileLayer",{
  ...
  Q.component('aiShoot', {
    ...
    added: function() {
      this.entity.on("step",this,"seekPlayer");
      this.entity.p.moveSpeed = 50;
      this.entity.p.facing = "";
    },

    seekPlayer: function(elapsedTipe) {

      if(this.entity.p.ignoreControls === undefined || !this.entity.p.ignoreControls) {
        
        this.entity.p.vx = 0;        
        this.entity.p.vy = 0;

        var player = Q("Player").first();
        var diffX =  player.p.x - this.entity.p.x;
        var diffY = player.p.y - this.entity.p.y;

        var marginX =  player.p.cx + this.entity.p.cx - 10;
        var marginY = player.p.cy + this.entity.p.cy - 10;

        if(Math.round(Math.abs(diffX)) < marginX || Math.round(Math.abs(diffY) < marginY)){
        	if( this.entity.p.animation != "ready" && this.entity.p.animation != "shoot"){
  	        this.entity.p.vx = 0;
  	        this.entity.p.vy = 0;
  	        this.entity.p.x = Math.round(this.entity.p.x);
  	        this.entity.p.y = Math.round(this.entity.p.y);
  	        if(Math.round(Math.abs(diffX)) < marginX) { //aim up or down	
  	        	if(diffY > 0){
  	      			this.entity.sheet("octorok_red_shoot_d");
  	      			this.entity.p.facing = "down";
  					this.entity.play("ready");
  	        	}
  	        	else{  		
  	      			this.entity.sheet("octorok_red_shoot_u");
  	            	this.entity.play("ready");
  	      			this.entity.p.facing = "up";
  	            }
    	  		}
    	  		else {//aim left or right
    	        if(diffX > 0){
    	      		this.entity.sheet("octorok_red_shoot_r");
    					  this.entity.play("ready");
    	      			this.entity.p.facing = "right";
  	        	}
  	        	else{		
  	      			this.entity.sheet("octorok_red_shoot_l");
  	            this.entity.play("ready");
  	      			this.entity.p.facing = "left";
  	          }
    	  		}
    	  	}
        }
        else{
        	this.entity.sheet("octorok_red");
          if(Math.abs(diffX) < Math.abs(diffY)) { //go left/right
            this.entity.p.vy = 0;
  	        this.entity.p.y = Math.round(this.entity.p.y);
            if(diffX > 0) { //right
              this.entity.p.vx = this.entity.p.moveSpeed;
              this.entity.play("move_right");
            }
            else {  //left
              this.entity.play("move_left");
              this.entity.p.vx = - this.entity.p.moveSpeed;
            }
          }
          else{                                   //go up/down
            this.entity.p.vx = 0;
  	          this.entity.p.x = Math.round(this.entity.p.x);
            if(diffY > 0) { //down
              this.entity.play("move_down");
              this.entity.p.vy = this.entity.p.moveSpeed;
            }
            else {  //up
              this.entity.play("move_up");
              this.entity.p.vy = - this.entity.p.moveSpeed;
            }
          }
        }
      ...
      if(this.entity.p.ignoreControls === undefined || !this.entity.p.ignoreControls) {
        ...
        if(Math.round(Math.abs(diffX)) < marginX || Math.round(Math.abs(diffY) < marginY)){
          ...
          if(Math.abs(diffX) > Math.abs(diffY)) { //go left/right
            ...
            this.entity.p.y = Math.round(this.entity.p.y);
            if(diffX > 0) { //right
              this.entity.customplay("skeleton_move_right", "move");
              this.entity.p.vx = this.entity.p.moveSpeed;
            }
            else {  //left              
              this.entity.customplay("skeleton_move_left", "move");
              this.entity.p.vx = - this.entity.p.moveSpeed;
            }
          }
          else{                                   //go up/down
            ...
            this.entity.p.x = Math.round(this.entity.p.x);
            if(diffY > 0) { //down
              this.entity.customplay("skeleton_move_down", "move");
              this.entity.p.vy = this.entity.p.moveSpeed;
            }
            else {  //up
              this.entity.customplay("skeleton_move_up", "move");
              this.entity.p.vy = - this.entity.p.moveSpeed;
            }
          }
        }
